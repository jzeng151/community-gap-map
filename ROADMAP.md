...
- [x] 2.1 Audit NYC Open Data Facilities & Providers API — field coverage by borough/category; document mapping in NOTES.md
...
- [x] 2.4 Write Python import script (`scripts/seed_offerings.py`) — normalize fields, classify `provider_type`, dedupe on `(name, address, category)`
- [x] 2.5 Hours quality check — FacDB has no hours (0%); all records get `availability_status = 'unknown'`
- [x] 2.6 Set `availability_status` at import time; record `imported_at` timestamp
- [ ] 2.7 Run one-time seed into Supabase via service role key; verify row count + spot-check per borough
- [x] 2.8 Re-import procedure documented in NOTES.md (monthly; which fields refresh vs. preserve)
